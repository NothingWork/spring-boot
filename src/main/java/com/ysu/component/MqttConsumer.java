 ...
 * @author Yun
 * @version 1.0
 * @description: 实现ApplicationRunner接口的run方法以实现在项目启动后执行所需要的操作
 * @date 2023/9/4
 */
@Component
public class MqttConsumer implements ApplicationRunner {
    private static MqttClient client;
    private static MqttConsumerCallback mqttConsumerCallback;
    //重写run方法
    @Override
    ...
    private void connect() {
        try {
            ...
            // 4 最后设置
            create(options, topic, qos,message);
        } catch (Exception e) {
    ...
    /**
     * 生成配置对象，用户名，密码等，这些属性从propertiesUtil中获取
     */
    ...
    /**
     * qos   为每个默认连接topic生成对应的qos，均设置为0
     */
    public int[] getQos(int length) {
        ...
        System.out.println("--设置消息发布质量"+ qos[0]);
        return qos;
    }
     ...
     * 装载各种实例和订阅主题  --- 4 ---
     */
    public void create(MqttConnectOptions options, String[] topic, int[] qos,MqttMessage message) {
        try {
            mqttConsumerCallback = new MqttConsumerCallback(client, options, topic, qos, message);
    ...
    public static MqttMessage getMessage(){
        return mqttConsumerCallback.getMessage();
    }
}
